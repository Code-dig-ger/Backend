...
def sendMailToUsers(rating_changes, new_contest):
    ...
    subject = 'Sending Mail for Rating Change ' + \
    ...
    for rating_change in rating_changes:
        ...
        if user_profile.exists():
            ...
            if codeforces_user.exists():
                ucr = user_contest_rank.objects.filter(
                    user=codeforces_user[0], contest=new_contest)
                if ucr.exists():
...
def codeforces_update_problems():
    ...
    for codeforces_contest in data['result'][:100]:
        ...
        url = "https://codeforces.com/api/contest.standings?contestId=" + \
            str(codeforces_contest['id']) + "&from=1&count=1"
        res = requests.get(url)
        ...
        for contest_problem in data['result']['problems']:
            ...
            new_problem.url = "https://codeforces.com/contest/" + \
                str(contest_problem['contestId']) + "/problem/" + contest_problem['index']
    ...
    for codeforces_contest in data['result'][-100:]:
        ...
        for contest_problem in data['result']['problems']:
            ...
            new_problem.name = contest_problem['name']
            new_problem.contest_id = contest_problem['contestId']
            new_problem.url = "https://codeforces.com/gym/" + \
                str(contest_problem['contestId']) + "/problem/" + contest_problem['index']
            new_problem.index = contest_problem['index']
            new_problem.tags = contest_problem['tags']
            if 'rating' in contest_problem:
...
def codeforces_update_contest():
    ...
    for codeforces_contest in data["result"][-50:]:
        ...
        if len(
            user_contest_rank.objects.filter(
                contest=new_contest)) == len(
                data['result']):
            continue
