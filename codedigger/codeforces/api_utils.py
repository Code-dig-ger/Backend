...
def profile_status(handle):
        ...
        if is_verdict_ok(submission):
            ...
            if not is_contest_gym(submission["contestId"]):
                if index in problems_solved_index:
                    problems_solved_index[index].append(
                        get_prob_id(submission))
                else:
                    problems_solved_index[index] = [get_prob_id(submission)]

                if submission["problem"]["rating"] in problems_solved_rating:
                    problems_solved_rating[submission["problem"]
                                           ["rating"]].append(
                                               get_prob_id(submission))
                else:
                    problems_solved_rating[submission["problem"]["rating"]] = [
                        get_prob_id(submission)
                    ]
                ...
                if index in problems_solved_index_gym:
                    problems_solved_index_gym[index].append(
                        get_prob_id(submission))
                else:
                    problems_solved_index_gym[index] = [
                        get_prob_id(submission)
                    ]
        else:
    ...
    return (problems_solved_rating, problems_solved_index,
            problems_solved_index_gym, number_of_upsolved,
            problems_solved_contest, correct_submissions,
