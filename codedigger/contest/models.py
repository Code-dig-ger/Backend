...
class Contest(models.Model):
    ...
    problem = models.ManyToManyField(
        Problem, through='ContestProblem', through_fields=(
            'contest', 'problem'), related_name='contest_problem')
    participants = models.ManyToManyField(
        User, through='ContestParticipation', through_fields=(
            'contest', 'user'), related_name='contest_participants')
...
class ContestResult(models.Model):
    ...
    contestParticipation = models.ForeignKey(
        ContestParticipation, on_delete=models.CASCADE)
    submissionTime = models.DurationField()
    penalty = models.IntegerField()

    def __str__(self):
        return self.contestParticipation.user.username + " " + \
            str(self.penalty) + " " + self.contestProblem.problem.prob_id + " " + self.contestProblem.contest.name
...
class CodeforcesContestSubmission(models.Model):
    participant = models.ForeignKey(
        CodeforcesContestParticipation,
        on_delete=models.CASCADE)
    ...
    def __str__(self):
        if self.isSolved:
            return self.participant.user.handle + " solved " + \
                self.participant.contest.name + " - " + str(self.problemIndex)
        else:
            return self.participant.user.handle + " not solved " + \
                self.participant.contest.name + " - " + str(self.problemIndex)
