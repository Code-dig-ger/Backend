...
def get_problemSubmission(participant_handle, prob_id, startTime, endTime):
	...
	res = requests.get("https://codeforces.com/api/user.status?handle="+participant_handle)
	...
	for submission in res["result"]:
		if 'contestId' in submission['problem'] and 'verdict' in submission: 
			...
			submissionTime = datetime.utcfromtimestamp(creationTime).replace(tzinfo = pytz.UTC)
			...
			if submissionTime >= startTime and submissionTime <= endTime :
				if problem_id in prob_id and submission['passedTestCount'] > 0 : 
					if submission['verdict'] == 'OK' and problemSubmission[problem_id] == None:
...
def prepareResult(contest):
	...
	for participant in participants :
		...
		participant_handle = Profile.objects.get(owner = participant.user).codeforces
		problemSubmission = get_problemSubmission(participant_handle, prob_id, contest.startTime, contest.startTime + contest.duration) 
