        ...
        for guru in gurus:
            ...
            for submission in submissions_guru:
                ...
                if 'contestId' in submission['problem']:
                    ...
                    if str(submission["problem"]['contestId']) + \
...
class SolveProblemsAPIView(
    ...
    def get(self, request):
        ...
        return JsonResponse(
            {'status': 'OK', 'result': ProbSerializer(problem_qs, many=True).data})
...
class ProblemSolvedByFriend(generics.GenericAPIView):
    ...
    def get(self, request, prob_id):
        ...
        userSolved = Solved.objects.filter(
            problem=problem).values_list(
            'user', flat=True)
        ...
        friendSolvedByRequest = FriendsShowSerializer(
            friendSolvedByRequest, context={
                'by_to_user': True}, many=True).data
        friendSolvedByAccept = FriendsShowSerializer(
            friendSolvedByAccept, context={
    ...
    def get(self, request):
        ...
        if handle == "" or handle is None:
            return Response(
                {
                    'status': 'FAILED',
                    'error': 'Please activate your account once by putting your name and codeforces handle..'},
                status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request):
        ...
        if handle == "" or handle is None:
            return Response(
                {
                    'status': 'FAILED',
                    'error': 'You haven\'t Entered your Codechef Username in your Profile.. Update Now!'},
                status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request):
        ...
        if handle == "" or handle is None:
            return Response(
                {
                    'status': 'FAILED',
                    'error': 'You haven\'t Entered your Atcoder Handle in your Profile.. Update Now!'},
                status=status.HTTP_400_BAD_REQUEST)
