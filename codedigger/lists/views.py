...
class TopicwiseGetListView(generics.ListAPIView):
    ...
    queryset = List.objects.filter((Q(type_list='1') | Q(type_list='3')) & Q(
        isTopicWise=True) & Q(public=True) & Q(isAdmin=True))
...
class TopicWiseRetrieveView(views.APIView):
    ...
    def get_object(self, slug):
        if List.objects.filter((Q(type_list='1') | Q(type_list='3')) & Q(
                isTopicWise=True) & Q(public=True) & Q(isAdmin=True) & Q(slug=slug)).exists():
...
class TopicwiseGetLadderView(generics.ListAPIView):
    ...
    permission_classes = [AuthenticatedOrReadOnly]
    queryset = List.objects.filter((Q(type_list='2') | Q(type_list='3')) & Q(
        isTopicWise=True) & Q(public=True) & Q(isAdmin=True))
...
class TopicWiseLadderRetrieveView(generics.RetrieveAPIView):
    ...
    def get_object(self, slug):
        if List.objects.filter((Q(type_list='2') | Q(type_list='3')) & Q(
                isTopicWise=True) & Q(public=True) & Q(isAdmin=True) & Q(slug=slug)).exists():
            return List.objects.get(slug=slug)
        raise NotFoundException("The list with the given slug does not exist")
    ...
    def get(self, request, slug):
        ...
        path = request.build_absolute_uri(
            '/lists/topicwise/ladder/' + str(slug) + '?')
        user = self.request.user
        if user.is_anonymous:
            user = None
        if user is not None:
            if not LadderStarted.objects.filter(
                    ladder_user=user, ladder=curr_list).exists():
                LadderStarted.objects.create(
                    ladder_user=user, ladder=curr_list)
        problem_qs = curr_list.problem.all()
        if user:
            temp = ['F']
            if Profile.objects.get(owner=user).spoj is not None:
                temp.append('S')
            if Profile.objects.get(owner=user).uva_handle is not None:
                temp.append('U')
            if Profile.objects.get(owner=user).atcoder is not None:
                temp.append('A')
            if Profile.objects.get(owner=user).codechef is not None:
                temp.append('C')
...
class LevelwiseGetListView(generics.ListAPIView):
    ...
    permission_classes = [AuthenticatedOrReadOnly]
    queryset = List.objects.filter((Q(type_list='1') | Q(type_list='3')) & Q(
        isTopicWise=False) & Q(public=True) & Q(isAdmin=True))
...
class LevelwiseRetrieveView(views.APIView):
    ...
    def get_object(self, slug):
        if List.objects.filter((Q(type_list='1') | Q(type_list='3')) & Q(
                isTopicWise=False) & Q(public=True) & Q(isAdmin=True) & Q(slug=slug)).exists():
            return List.objects.get(slug=slug)
        raise NotFoundException("The list with the given slug does not exist")
...
class LevelwiseLadderRetrieveView(generics.RetrieveAPIView):
    ...
    def get(self, request, slug):
        ...
        path = request.build_absolute_uri(
            '/lists/levelwise/ladder/' + str(slug) + '?')
        user = self.request.user
        if user.is_anonymous:
            user = None
        if user is not None:
...
# return response.Response({'status' : "OK",'result' : 'ladder
...
# return response.Response(data={'status' : 'OK','result' :'list
# updated'})
...
class EditUserlistView(generics.GenericAPIView):
    ...
    def get_object(self, slug):
        if self.request.user.is_staff:
            if List.objects.filter((Q(isAdmin=True) | Q(
                    owner=self.request.user)) & Q(slug=slug)).exists():
                return List.objects.get((Q(isAdmin=True) | Q(
                    owner=self.request.user)) & Q(slug=slug))
            else:
                raise NotFoundException(
                    "The list with the given slug does not exist")
        else:
            if List.objects.filter(
                    Q(owner=self.request.user) & Q(slug=slug)).exists():
    ...
    def put(self, request, slug):
        ...
        if name is not None:
            if List.objects.filter(
                ...
                return response.Response(
                    {
                        "status": 'FAILED',
                        'error': "You already have a created a list with the same name "},
                    status=status.HTTP_400_BAD_REQUEST)
            else:
                curr_list.name = name
        ...
        if public is not None:
            if public is not True and public is not False:
                return response.Response(
                    {
                        "status": 'FAILED',
                        'error': "public field can only be true or false (with the lowercase initial character)"},
                    status=status.HTTP_400_BAD_REQUEST)
        ...
        if data.get('delete_probs', None):
            for ele in data.get('delete_probs', None):
                ...
                if not Problem.objects.filter(
                        prob_id=prob_id, platform=platform).exists():
                    return response.Response(
                        {
                            "status": 'FAILED',
                            'error': "Problem with the prob_id " + ele + " and platform " + platform + " does not exist"},
                        status=status.HTTP_400_BAD_REQUEST)
                curr_prob = Problem.objects.get(
                    prob_id=prob_id, platform=platform)
                if not curr_list.problem.filter(
                        prob_id=prob_id, platform=platform).exists():
                    return response.Response(
                        {
                            "status": 'FAILED',
                            'error': "Problem with the given prob_id " + ele + " does not exists within the list"},
                        status=status.HTTP_400_BAD_REQUEST)
...
class AddProblemsAdminView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return response.Response(
            {
                "status": 'OK',
                'result': "The correct problems have been inserted in the list",
                'wrong': wrong,
                'double': double,
                'rating_l': rating_l,
                'rating_r': rating_r},
