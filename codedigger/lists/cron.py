...
def cron_codeforces(user):
    ...
    for ele in req['result']:
        if 'verdict' not in ele or 'contestId' not in ele or ele['verdict'] != 'OK':
            continue
        prob_id = str(ele['problem']['contestId']) + \
            str(ele['problem']['index'])
        prob = Problem.objects.filter(prob_id=prob_id, platform='F')
        if not prob.exists():
            continue
        solve, created = Solved.objects.get_or_create(
            user=user, problem=prob[0])
        if not created:
            limit -= 1
            if limit <= 0:
                break
...
def uva_updater():
    send_mail(msg.format('Uva', 'Started'), 'OK', EMAIL_HOST_USER,
              ['testing.codedigger@gmail.com'], fail_silently=True)
    for ele in User.objects.all():
        cron_uva(ele)
    send_mail(msg.format('Uva', 'Finished'), 'OK', EMAIL_HOST_USER,
              ['testing.codedigger@gmail.com'], fail_silently=True)
...
def spoj_updater():
    send_mail(msg.format('Spoj', 'Started'), 'OK', EMAIL_HOST_USER,
              ['testing.codedigger@gmail.com'], fail_silently=True)
    for ele in User.objects.all():
        cron_spoj(ele)
    send_mail(msg.format('Spoj', 'Finished'), 'OK', EMAIL_HOST_USER,
              ['testing.codedigger@gmail.com'], fail_silently=True)
