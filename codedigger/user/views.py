...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        return Response({'status': "OK", 'result': user_data},
...
class ChangePassword(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(new_pass) < 6:
            return Response({'status': 'FAILED',
                             'error': "The password is too short, should be of minimum length 6"},
                            status=status.HTTP_400_BAD_REQUEST)
        if user is None:
            return Response({'status': 'FAILED',
        ...
        if User.objects.filter(email=email).exists():
            ...
            if user.auth_provider != 'email':
                return Response(
                    {
                        'status': 'Failed',
                        'error': 'You cannot reset password if you registered with google'},
                    status=status.HTTP_400_BAD_REQUEST)
                ...
                if redirect_url and len(redirect_url) > 3:
                    ...
                    return redirect(
                        os.getenv(
                            ...
                            '') + '?token_valid=False')
                return Response(
                    {'status': 'FAILED', 'error': 'Token is invalid. Please request a new one'})
            if redirect_url and len(redirect_url) > 3:
                return redirect(
                    redirect_url +
...
class SearchUser(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        profiles = Profile.objects.filter(
            owner__is_verified=True).exclude(
            name=None).exclude(
            codeforces=None)
            ...
            if request.user.is_authenticated:
                if request.user.username == profile.owner.username:
                    ...
                    if UserFriends.objects.filter(
                        status=True, to_user=request.user, from_user=User.objects.get(
    ...
    def get(self, request, username):
        ...
        try:
            ...
            return Response(
                {
                    'status': 'FAILED',
                    'error': 'Requested User doesn\'t exists in our database. Register Now! :)'},
                status=status.HTTP_400_BAD_REQUEST)

        profile = Profile.objects.get(owner=user)

        if profile.codeforces is None:
            return Response({'status': 'FAILED',
                             'error': 'Requested User haven\'t activated his/her account. :( '},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if request.GET.get('platform') is None:
            ...
            if codeforces_user is not None:
            ...
            if profile.codechef != "" and profile.codechef is not None:
                data = get_codechef_profile(profile.codechef)
            else:
                return Response({'status': 'FAILED', 'error': ermsg.format(
                    'codechef')}, status=status.HTTP_400_BAD_REQUEST)

        elif request.GET.get('platform') == "atcoder":
            if profile.atcoder != "" and profile.atcoder is not None:
                data = get_atcoder_profile(profile.atcoder)
                data['solvedCount'] = Solved.objects.filter(
                    user=profile.owner, problem__platform='A').count()
            else:
                return Response({'status': 'FAILED', 'error': ermsg.format(
                    'atcoder')}, status=status.HTTP_400_BAD_REQUEST)

        elif request.GET.get('platform') == "uva":
            if profile.uva_handle != "" and profile.uva_handle is not None:
                data = get_uva_profile(profile.uva_id, profile.uva_handle)
            else:
                return Response({'status': 'FAILED', 'error': ermsg.format(
                    'uva')}, status=status.HTTP_400_BAD_REQUEST)

        elif request.GET.get('platform') == "spoj":
            if profile.spoj != "" and profile.spoj is not None:
                data = get_spoj_profile(profile.spoj)
            else:
                return Response({'status': 'FAILED', 'error': ermsg.format(
                    'spoj')}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({'status': 'FAILED',
...
class SendFriendRequest(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if Profile.objects.get(
                ...
                owner=request.user).codeforces is None:
            return Response(
                {
                    'status': 'FAILED',
                    'error': 'You have not activated your account. Please activate your account by putting your name and codeforces handle in your profile.. '},
                status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if Profile.objects.get(
                    ...
                    owner=to_user).codeforces is None:
                return Response({'status': 'FAILED',
                                 'error': 'Requested User have not activated his account.'},
                                status=status.HTTP_400_BAD_REQUEST)
        except User.DoesNotExist:
            return Response({'status': 'FAILED',
        ...
        try:
            ...
            try:
                ...
                if uf.status:
                    ...
                    status.status = True
                    return Response(
                        {'status': 'OK', 'result': 'You are now Friends'})

            except UserFriends.DoesNotExist:
                UserFriends.objects.create(
                    from_user=request.user, to_user=to_user, status=False)
                return Response(
                    {'status': 'OK', 'result': 'Friend Request Sent!'})
    ...
    def post(self, request):
        ...
        try:
            ...
            if Profile.objects.get(
                    ...
                    owner=user).codeforces is None:
                return Response({'status': 'FAILED',
                                 'error': 'Requested User haven\'t activated his account.'},
                                status=status.HTTP_400_BAD_REQUEST)
        except User.DoesNotExist:
            return Response({'status': 'FAILED',
        ...
        try:
            ...
            try:
                opp_status = UserFriends.objects.get(
            ...
            uf.delete()
            return Response(
                {'status': 'OK', 'result': 'Removed Successfully!'})
        except UserFriends.DoesNotExist:
            try:
                opp_status = UserFriends.objects.get(
                    from_user=user, to_user=request.user)
    ...
    def put(self, request):
        ...
        if Profile.objects.get(
            ...
            return Response(
                {
                    'status': 'FAILED',
                    'error': 'You have\'n activated your account. Please activate your account by putting your name and codeforces handle in your profile.. '},
                status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if Profile.objects.get(
                    ...
                    owner=from_user).codeforces is None:
                return Response({'status': 'FAILED',
                                 'error': 'Requested User haven\'t activated his account.'},
                                status=status.HTTP_400_BAD_REQUEST)
        except User.DoesNotExist:
        ...
        try:
            ...
            return Response({'status': 'FAILED',
                             'error': 'No Request Found! It seems User have removed Request.'},
                            status=status.HTTP_400_BAD_REQUEST)
...
class FriendsShowView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        friendsbyrequest = UserFriends.objects.filter(
            status=True, from_user=request.user)
        friendsbyaccept = UserFriends.objects.filter(
            status=True, to_user=request.user)

        friendsbyrequest = FriendsShowSerializer(
            friendsbyrequest, context={
                'by_to_user': True}, many=True).data
        friendsbyaccept = FriendsShowSerializer(
            friendsbyaccept, context={
                'by_to_user': False}, many=True).data
...
class FriendRequestShowView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        friendsbyaccept = UserFriends.objects.filter(
            status=False, to_user=request.user)
        friendsbyaccept = FriendsShowSerializer(
            friendsbyaccept, context={
                'by_to_user': False}, many=True).data
...
class RequestSendShowView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        friendsbyrequest = UserFriends.objects.filter(
            status=False, from_user=request.user)
        friendsbyrequest = FriendsShowSerializer(
            friendsbyrequest, context={
                'by_to_user': True}, many=True).data
