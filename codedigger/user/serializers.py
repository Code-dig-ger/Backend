    ...
    def validate(self, attrs):
        ...
        if res.status_code >= 500:
            ...
            raise ValidationException(
                handle + ' is not a valid Codeforces handle')
    ...
    def validate(self, attrs):
        ...
        if user_obj_email:
            user = auth.authenticate(
                username=user_obj_email.username,
                password=password)
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    spoj = serializers.CharField(
        allow_blank=True,
        allow_null=True,
        required=False)
    codechef = serializers.CharField(
        allow_blank=True,
        allow_null=True,
        required=False)
    atcoder = serializers.CharField(
        allow_blank=True,
        allow_null=True,
        required=False)
    uva_handle = serializers.CharField(
    ...
    def validate_codeforces(self, value):
        ...
        if value != Profile.objects.get(owner=curr_user).codeforces:
            ...
            if cf_status == 2:
                Solved.objects.filter(
                    user=curr_user, problem__platform='F').delete()
            elif cf_status:
                raise ValidationException(
    ...
    def validate_codechef(self, value):
        ...
        if value != Profile.objects.get(owner=curr_user).codechef:
            if check_handle_codechef(value):
                Solved.objects.filter(
                    user=curr_user, problem__platform='C').delete()
            else:
                raise ValidationException(
    ...
    def validate_atcoder(self, value):
        ...
        if value != Profile.objects.get(owner=curr_user).atcoder:
            if check_handle_atcoder(value):
                Solved.objects.filter(
                    user=curr_user, problem__platform='A').delete()
            else:
                raise ValidationException(
    ...
    def validate_spoj(self, value):
        ...
        if value != Profile.objects.get(owner=curr_user).spoj:
            if check_handle_spoj(value):
                Solved.objects.filter(
                    user=curr_user, problem__platform='S').delete()
            else:
                raise ValidationException(
    ...
    def validate_uva_handle(self, value):
        ...
        if value != Profile.objects.get(owner=curr_user).uva_handle:
            if check_handle_uva(value):
                Solved.objects.filter(
                    user=curr_user, problem__platform='U').delete()
            else:
                raise ValidationException(
...
class PasswordChangeSerializer(serializers.Serializer):
    old_pass = serializers.CharField(
        max_length=68, min_length=6, required=True)
