...
from .views import (RegisterView, VerifyEmail, LoginApiView, ProfileGetView,
                    ProfileUpdateView, PasswordTokenCheckAPI,
                    RequestPasswordResetEmail, SetNewPasswordAPIView,
                    UserProfileGetView, ChangePassword, SendVerificationMail,
                    CheckAuthView, SearchUser)
# Friend Related View
...
urlpatterns = [
    ...
    path('user/show-request',
         FriendRequestShowView.as_view(),
         name='Show_Friend_Request_List'),
    path('user/show-send-request',
         RequestSendShowView.as_view(),
         name='Show_Friend_Request_Send_List'),
