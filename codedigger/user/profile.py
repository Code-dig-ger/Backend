...
def get_atcoder_profile(handle):
    ...
    if grp is None:
        data['color'] = soup.find(
            'a', {'class': 'username'}).find('span').get('style')
    else:
        data['color'] = ColorCode[grp[0]]

    if grp is not None and 'unrated' in grp[0]:
        data['rating'] = 'UnRated'
    elif soup.find('div', {'class': 'col-md-3 col-sm-12'}).find('b') is None:
        data['rank'] = '20 Kyu'
    else:
        data['rank'] = soup.find(
            'div', {'class': 'col-md-3 col-sm-12'}).find('b').text
    ...
    if data['rating'] != 'UnRated':
        ...
        data['maxColor'] = ColorCode[details[2].findAll('span')[
            0].get('class')[0]]
    ...
    if contestTable is not None:
        ...
        for contest in contests:
            ...
            contest_detail['url'] = base_url + \
                contest.findAll('td')[1].find('a')['href']
            contest_detail['code'] = contest.findAll('td')[1].find('a')[
                'href'].split('/')[-1]
            contest_detail['standing_url'] = base_url + \
                contest.findAll('td')[2].find('a')['href']
        ...
        data['contestRank'] = sorted(
            contests_details,
            key=lambda i: i['worldRank'])[
            :3]
...
def get_spoj_profile(handle):
    ...
    data['solvedCount'] = soup.find(
        'dl', {
            'class': 'dl-horizontal profile-info-data profile-info-data-stats'}).find('dd').text

    rank = None
    y = soup.find('div', {'id': 'user-profile-left'}).findAll('p')
    del soup
    for x in y:
        if x.find('i', {'class': 'fa fa-trophy'}) is not None:
            rank = x.text
            break
...
def get_codechef_profile(handle):
    ...
    data['maxRating'] = int(
        soup.find(
            'div', {
                'class': 'rating-header'}).find('small').text.replace(
            '(Highest Rating ', '').replace(
                    ')', ''))
    ...
    data['solvedCount'] = problems_solved.find(
        'h5').text.replace('Fully Solved (', '').replace(')', '')
    data['status'] = 'OK'
    return data
